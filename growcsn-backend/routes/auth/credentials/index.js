...
const {
  ...
  authCheckPostCredentialsusernameRegisterUser,
...
module.exports = () => {
  ...
  router.post("/", rateLimiterStrictMiddleware, async (req, res) => {
    try {
      ...
      let userDatabase = await User.findOne({ "local.email": email })
        .select("local ips")
        .lean();
      ...
      userDatabase = await User.findByIdAndUpdate(userDatabase._id, {
        ...
        .select(
          "local.email local.emailVerified roblox.id discord.id username avatar rank balance xp vault stats rakeback fair anonymous mute ban verifiedAt updatedAt createdAt"
        )
        .lean();
  ...
  router.post("/register", rateLimiterStrictMiddleware, async (req, res) => {
    try {
      ...
      // Get user from database
      const usernamebase = await User.findOne({ "local.username": username })
        .select("local")
        .lean();

      // Validate user
      authCheckPostCredentialsusernameRegisterUser(usernamebase);
      ...
      res
        .status(200)
        .json({ success: true, token: accessToken, user: dataDatabase[0] });
    } catch (err) {
      res
        .status(500)
        .json({
    ...
    try {
      ...
      const message =
        req.body.type === "verify"
          ? `Please verify your e-mail address on ${
              process.env.SERVER_FRONTEND_URL.split(",")[0]
            } by clicking on the following link: \n\n${
              process.env.SERVER_FRONTEND_URL.split(",")[0]
            }/verify?userId=${userDatabase._id}&token=${token}`
          : `Please reset your password on ${
              process.env.SERVER_FRONTEND_URL.split(",")[0]
            } by clicking on the following link: \n\n${
              process.env.SERVER_FRONTEND_URL.split(",")[0]
            }/reset?userId=${userDatabase._id}&token=${token}`;
